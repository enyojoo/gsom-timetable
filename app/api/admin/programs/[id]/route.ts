...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { degree_id, name_en, name_ru, code } = await request.json()
    const programId = Number.parseInt(params.id)

    if (!degree_id || !name_en || !name_ru || !code) {
      return NextResponse.json({ success: false, message: "All fields are required" }, { status: 400 })
    }

    const result = await sql`
      UPDATE programs 
      SET degree_id = ${degree_id}, name_en = ${name_en}, name_ru = ${name_ru}, code = ${code}, updated_at = CURRENT_TIMESTAMP
      WHERE id = ${programId}
      RETURNING id, degree_id, name_en, name_ru, code, created_at, updated_at
    `
