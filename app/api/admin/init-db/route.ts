...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create tables (this will be idempotent due to IF NOT EXISTS)
    await sql`
    ...
    const defaultAdminResult = await createAdminUser(
      "admin",
      "admin@gsom.spbu.ru",
      process.env.ADMIN_PASSWORD || "gsom-admin-2024",
      "super_admin",
    )
    ...
    return NextResponse.json({
      ...
      adminCreated: defaultAdminResult.success,
    })
  } catch (error) {
